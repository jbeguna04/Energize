...
public class NotificationsPreferenceFragment extends PreferenceFragment {
	...
	private final ServiceConnection monitorServiceConnection = new ServiceConnection() {
		...
		public void onServiceConnected(final ComponentName className, final IBinder service) {
			NotificationsPreferenceFragment.this.monitorService = new Messenger(service);
			try {
				Log.d(NotificationsPreferenceFragment.TAG, "Trying to connect to the battery monitoring service...");
				final Message msg = Message.obtain(null, MonitorBatteryStateService.MSG_REGISTER_CLIENT);
				msg.replyTo = NotificationsPreferenceFragment.this.monitorServiceMessanger;
				NotificationsPreferenceFragment.this.monitorService.send(msg);
			} catch (final RemoteException e) {
				Log.e(NotificationsPreferenceFragment.TAG, "Failed to connect to the battery monitoring service!");
			}
		}

		public void onServiceDisconnected(final ComponentName className) {
			NotificationsPreferenceFragment.this.monitorService = null;
		}
	};
	...
	private String getSoftwareVersion() {
		try {
			...
			Log.e(NotificationsPreferenceFragment.TAG, "Package name not found", e);
		}
		return "N/A";
	}
	...
	public void onCreate(final Bundle savedInstanceState) {
		...
		sendDatabasePreference.setOnPreferenceClickListener(new OnPreferenceClickListener() {
			...
			public boolean onPreferenceClick(final Preference preference) {
				Log.v(NotificationsPreferenceFragment.TAG, "Clearing battery statistics database...");
				try {
					final Message msg = Message.obtain(null, MonitorBatteryStateService.MSG_CLEAR_STATISTICS);
					msg.replyTo = NotificationsPreferenceFragment.this.monitorServiceMessanger;
					NotificationsPreferenceFragment.this.monitorService.send(msg);
				} catch (final RemoteException e) {
					Log.e(NotificationsPreferenceFragment.TAG, "Failed to clear the battery statistics database!");
				}
				return false;
			}
		});
		...
		public void handleMessage(final Message msg) {
			switch (msg.what) {
				case MonitorBatteryStateService.MSG_CLEAR_STATISTICS:
					final AlertDialog.Builder builder = new AlertDialog.Builder(NotificationsPreferenceFragment.this.getActivity());
				...
				case MonitorBatteryStateService.MSG_COPY_DB_TO_SDCARD:
					final AlertDialog.Builder builderCopy = new AlertDialog.Builder(NotificationsPreferenceFragment.this.getActivity());
