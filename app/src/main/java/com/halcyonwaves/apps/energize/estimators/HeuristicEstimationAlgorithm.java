 ...
 * @since 2.1
 */
public class HeuristicEstimationAlgorithm {
	 ...
	 */
	private final static String TAG = "HeuristicEstimationAlg";
	...
	public static EstimationResult getEstimation(final Context context) {
		...
		long[] timeDifferences = new long[HeuristicEstimationAlgorithm.MAX_N];
		for (int i = 0; i < HeuristicEstimationAlgorithm.MAX_N; ++i) {
		...
		Log.v(HeuristicEstimationAlgorithm.TAG, String.format("Calculated the time between the last two (%d, %d) events: %d", lastEventTime, prevEventTime, timeDifferences[0]));
		...
		for (int currentStep = 1; currentStep < HeuristicEstimationAlgorithm.MAX_N; ++currentStep) {
		...
		for (; numberOfValidEvents < HeuristicEstimationAlgorithm.MAX_N; ++numberOfValidEvents) {
		...
		if (RawBatteryStatisicsTable.CHARGING_STATE_DISCHARGING == lastChargingState) {
			...
			Log.v(HeuristicEstimationAlgorithm.TAG, String.format("Calculated remaining battery life in minutes: %d", lastRemainingMinutes));
		} else {
			lastRemainingMinutes = (Math.round(((100.0f - lastChargingPercentage) * averageTimeDifference) / 60.0f));
			Log.v(HeuristicEstimationAlgorithm.TAG, String.format("Calculated remaining charging time in minutes: %d", lastRemainingMinutes));
		}
