...
public class EnergizeExtension extends DashClockExtension {
	...
	private void sendPublishedData() {
		...
		if (this.remainingTimeEstimation.isValid) {
			this.publishUpdate(
					new ExtensionData().visible(true).icon(R.drawable.ic_battery).status(this.percentageLoaded + "%").expandedTitle(String.format(this.getApplicationContext().getString(R.string.dc_widget_charged_long), this.percentageLoaded))
